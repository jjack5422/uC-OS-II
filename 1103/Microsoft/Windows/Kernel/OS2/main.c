...
void task(void* p_arg) {
    ...
    INT32U next_period_start_time = task_data->TaskArriveTime;
    INT32U sleep_ticks;
    INT32U current_time;
    current_time = OSTimeGet();
    if (next_period_start_time > current_time) {
        OSTimeDly(next_period_start_time - current_time);
    }
    while (1) {
        //task_data->RemainTime;
        ...
        OS_ENTER_CRITICAL();
        next_period_start_time += task_data->TaskPeriodic; // T2: next_period = 10
        current_time = OSTimeGet(); // *** 這裡會抓到 t=10 ***
        if (current_time > next_period_start_time) { // if (10 >= 10)
            sleep_ticks = 0; // ** 得到正確的 sleep_ticks = 0 **
        }
        else {
            sleep_ticks = next_period_start_time - current_time;
        }
        OS_EXIT_CRITICAL()
        // (這會觸發 OS_Sched() 和 OSCtxSw())
        OSTimeDly(sleep_ticks);
        task_para_set* cur = OSTCBCur->OSTCBExtPtr;
        task_para_set* next = OSTCBHighRdy->OSTCBExtPtr;
        if (OSTCBCur->CompletedFlag == 1) {
            if (OSTCBHighRdy == OSTCBCur) {
                printf("%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t\n", OSTimeGet(), cur->TaskID, cur->job_no++, next->TaskID, next->job_no);
...
int  main(void)
{
    ...
    printf("\n\n");
    printf("Task\tArriveTime\tExecutionTime\tPeriodic\t\n");
    printf("{ 1           0               1              3}\n{ 2           0               3              5}");
    printf("\n\n");
