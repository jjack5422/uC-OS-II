...
static  void  OS_SchedNew(void)
{
    ...
    while (ptcb->OSTCBExtPtr && ptcb->OSTCBPrio !=OS_TASK_IDLE_PRIO)         /* Go through all TCBs in TCB list          */
    {

        if (((task_para_set*)(ptcb->OSTCBExtPtr))->deadline < earilestDeadline && currentTime >= ((task_para_set*)(ptcb->OSTCBExtPtr))->TaskArriveTime)                  //(currentTime >= ptcb->startTime) means this task is not finish
        {
            earilestDeadline = ((task_para_set*)(ptcb->OSTCBExtPtr))->deadline;
            choosePrio = ptcb->OSTCBPrio;
        }
        else if (((task_para_set*)(ptcb->OSTCBExtPtr))->deadline == earilestDeadline && currentTime >= ((task_para_set*)(ptcb->OSTCBExtPtr))->TaskArriveTime)            //(currentTime >= ptcb->startTime) means this task is not finish
        {
            if (ptcb == OSTCBCur)
            {
                earilestDeadline = ((task_para_set*)(ptcb->OSTCBExtPtr))->deadline;
