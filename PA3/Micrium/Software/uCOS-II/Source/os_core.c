                    ...
                    task_para_set* next = OSTCBHighRdy->OSTCBExtPtr;
                        ...
                        if (OSTCBCur->CompletedFlag != 1) {
                            if (OSTCBCur->OSTCBPrio != 63) {
                                ...
                                LOG_print(3, "./Output.txt", "%d\tPreemption\ttask(% 2d)(% 2d)\ttask(% 2d)(% 2d)\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID, next->job_no);
                            }
                            else {
                                LOG_print(3, "./Output.txt", "%d\tPreemption\ttask(%2d)\ttask(%2d)(%2d)\t\n", OSTimeGet(), OSTCBCur->OSTCBPrio, next->TaskID, next->job_no);
                            }
                        }
                    
                    if (OSTCBCur->CompletedFlag == 1) {

                        cur->ResponseTime = currentTime - cur->TaskArriveTime + 1;
                        cur->TaskArriveTime += cur->TaskPeriodic;
                        if (currentTime >= cur->TaskArriveTime) {
                            cur->Delay = 0;
                        }
                        else {
                            cur->Delay = cur->TaskArriveTime - currentTime;
                        }
                        ...
                        if (OSTCBHighRdy->OSTCBPrio != 63) {
                            LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID,
                                next->job_no,cur->ResponseTime-1, cur->BlockTime,cur->PreemptiveTime);
                            cur->job_no++;
                            cur->BlockTime = 0;
                            cur->PreemptiveTime = 0;
                            cur->Ready = 0;
                        }
                        else {
                            LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t%d\t%d\t%d\n", OSTimeGet(), cur->TaskID, cur->job_no,
                                OSTCBHighRdy->OSTCBPrio,cur->ResponseTime-1, cur->BlockTime,cur->PreemptiveTime);
                            cur->job_no++;
                            cur->BlockTime = 0;
                            cur->PreemptiveTime = 0;
                            cur->Ready = 0;
...
void  OSTimeTick(void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            ...
            task_para_set* next = OSTCBHighRdy->OSTCBExtPtr;
            currentTime = OSTimeGet();
            if (ptcb == OSTCBCur && ptcb->OSTCBExtPtr && ((task_para_set*)(ptcb->OSTCBExtPtr))->RemainTime > 0)  /* Check if this is the currently running task */
            {
                (((task_para_set*)(ptcb->OSTCBExtPtr))->RemainTime)--;                        	 		       /* Decrement compTime counter for the running task */
                if ((((task_para_set*)(ptcb->OSTCBExtPtr))->RemainTime) == 0) {
                    OSTCBCur->CompletedFlag = 1;
                }
            }
            if (ptcb->OSTCBPrio >= 1 && ptcb->OSTCBPrio <= 3 && currentTime >= ((task_para_set*)(ptcb->OSTCBExtPtr))->deadline && ((task_para_set*)(ptcb->OSTCBExtPtr))->RemainTime > 0)
            {
...
/* ========================================================================= */
#if PROTOCOL == NPCS
            // 確保 ptcb 有掛載擴充資料，且不是 IDLE Task
            if (ptcb->OSTCBExtPtr != (void*)0) {
                task_para_set* p_task_data = (task_para_set*)(ptcb->OSTCBExtPtr);

                // 統計條件：
                // 1. 任務必須是 READY 狀態 (OS_STAT_RDY)
                // 2. 任務不是當前正在執行的任務 (ptcb != OSTCBCur)
                // 3. 任務還沒執行完 (RemainTime > 0) (避免已完成但在等待下個週期的任務被誤算)

                if (ptcb->OSTCBStat == OS_STAT_RDY && ptcb != OSTCBCur && p_task_data->RemainTime > 0 && (currentTime > p_task_data->InitArriveTime) && p_task_data->Ready==1 && currentTime > p_task_data->TaskArriveTime) {

                    // 情境 A: Blocking Time
                    // 條件: 我 (ptcb) 的優先權比當前執行者 (OSTCBCur) 高 (數值小)，但我卻沒在跑。
                    // 原因: 在 NPCS 中，這代表 OSTCBCur 鎖住了排程器 (OSSchedLock)。
                    if (OSPrioCur != 63) {
                        if (ptcb->OSTCBPrio < OSTCBCur->OSTCBPrio) {
                            p_task_data->BlockTime++;
                            printf("%d\ttask(%2d)\tblockTime +1\n", OSTimeGet(), p_task_data->TaskID);
                        }
                        // 情境 B: Preemption Time
                        // 條件: 我 (ptcb) 的優先權比當前執行者 (OSTCBCur) 低 (數值大)。
                        // 原因: 因為有更重要的人在跑，所以我被迫等待。
                        else {
                            p_task_data->PreemptiveTime++;
                            printf("%d\ttask(%2d)\tpreemptiveTime +1\n", OSTimeGet(), p_task_data->TaskID);
                        }
                    }
                }
            }
            ...
            if (ptcb->OSTCBExtPtr != (void*)0) {
                ...
                // 基本過濾條件：
                // 1. 任務尚未執行完 (RemainTime > 0)
                // 2. 任務不是當前正在執行的任務
                // 3. 任務已到達 (Ready == 1 且時間已到)
                if (ptcb != OSTCBCur && p_task_data->RemainTime > 0 &&
                    (currentTime > p_task_data->InitArriveTime) &&
                    p_task_data->Ready == 1 &&
                    currentTime > p_task_data->TaskArriveTime) {

                    // 取得「當前執行任務 (Running Task)」的擴充資料，以獲取其【原始優先權】
                    // 注意：必須防呆，避免當前是 IDLE Task (Prio 63) 導致指標為 NULL
                    task_para_set* p_running_data = (task_para_set*)0;
                    if (OSTCBCur->OSTCBExtPtr != (void*)0) {
                        p_running_data = (task_para_set*)(OSTCBCur->OSTCBExtPtr);
                    }

                    // 如果目前跑的是 IDLE Task，那大家都不會被 Block，直接跳過不計
                    if (p_running_data != (task_para_set*)0) {

                        // 取得雙方的【原始優先權】 (Base Priority)
                        INT8U my_base_prio = p_task_data->TaskPriority;
                        INT8U running_base_prio = p_running_data->TaskPriority;

                        // === CPP 統計邏輯判定 ===

                        // 情況 A: 正在等 Mutex (OS_STAT_MUTEX)
                        // 定義：任務因為申請不到資源 (OSMutexPend) 而進入等待狀態。
                        // 這是最明確的 Blocking。
                        if (ptcb->OSTCBStat & OS_STAT_MUTEX) {
                            //p_task_data->BlockTime++;
                            // printf("%d\ttask(%2d)\tblockTime +1 (Mutex)\n", OSTimeGet(), p_task_data->TaskID);
                        }
                        // 情況 B: 任務是 READY 狀態，但沒輪到它跑
                        else if (ptcb->OSTCBStat == OS_STAT_RDY) {

                            // 這裡要判斷是「被搶佔 (Preemption)」還是「被阻擋 (Blocking/Priority Inversion)」
                            // 關鍵：比較【原始優先權】

                            // 如果目前跑的人，原始優先權比我低 (數值大)
                            // 代表發生了優先權反轉 (因為繼承了天花板優先權才跑贏我) -> Blocking
                            if (running_base_prio > my_base_prio) {
                                p_task_data->BlockTime++;
                                printf("%d\ttask(%2d)\tblockTime +1 (Prio Inv)\n", OSTimeGet(), p_task_data->TaskID);
                            }
                            // 如果目前跑的人，原始優先權本來就比我高 (數值小)
                            // 這是正常的排程行為 -> Preemption
                            else {
                                p_task_data->PreemptiveTime++;
                                printf("%d\ttask(%2d)\tpreemptiveTime +1\n", OSTimeGet(), p_task_data->TaskID);
                            }
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSTCBCur->CompletedFlag == 1 && cur->Delay == 0) {
                if (OSTCBHighRdy == OSTCBCur) {
                    LOG_print(3, "./Output.txt", "%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n", OSTimeGet(),
                        ...
                        cur->BlockTime, cur->PreemptiveTime);
                    OSTCBCur->CompletedFlag = 0;
                }
            }
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if (OSTCBCur->CompletedFlag != 1 && OSTimeGet()!=0) {
                    if (next != NULL ) {
                        LOG_print(3, "./Output.txt", "%d\tPreemption\ttask(% 2d)(% 2d)\ttask(% 2d)(% 2d)\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID, next->job_no);
                    }
                }
                if (OSTCBCur->CompletedFlag == 1) {
                    if (OSTCBHighRdy->OSTCBPrio != 63) {
                        if (next->RemainTime == 0) {
                            LOG_print(3, "./Output.txt", "%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t%d\t%d\t%d\n", OSTimeGet(), cur->TaskID, cur->job_no, 63,
                                cur->ResponseTime, cur->BlockTime, cur->PreemptiveTime);
                            cur->job_no++;
                            cur->BlockTime = 0;
                            cur->PreemptiveTime = 0;
                            cur->Ready = 0;
                            ...
                            LOG_print(3, "./Output.txt", "%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID,
                                next->job_no, cur->ResponseTime, cur->BlockTime, cur->PreemptiveTime);
                            cur->job_no++;
                            cur->BlockTime = 0;
                            cur->PreemptiveTime = 0;
                            cur->Ready = 0;
...
static  void  OS_SchedNew(void)
{
    ...
    INT8U   y;
    static INT8U is_first_check = 0u;

    if (is_first_check == 0u) {
        OS_TCB* ptcb;
        INT32U delay_ticks;
        ...
        ptcb = OSTCBList; /* 從 TCB 鏈結串列頭開始 */

        while (ptcb != (OS_TCB*)0 && (ptcb ->OSTCBExtPtr !=NULL)) {
            delay_ticks = ((task_para_set*)(ptcb->OSTCBExtPtr))->InitArriveTime;
            if (delay_ticks > 0) {
                ptcb->OSTCBDly = delay_ticks;

                /* 2. 從 Ready Table 中移除該任務 (Make task not ready) */
                /* 這是 OSTimeDly 內部做的核心動作 */
                if ((OSRdyTbl[ptcb->OSTCBY] &= ~ptcb->OSTCBBitX) == 0u) {
                    OSRdyGrp &= ~ptcb->OSTCBBitY;
                }
            }
            ptcb = ptcb->OSTCBNext; /* 移至下一個任務 */
        }
    }
