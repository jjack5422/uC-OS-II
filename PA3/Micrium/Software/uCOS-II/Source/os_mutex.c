    ...
    INT8U* perr)
{
...
#ifdef OS_SAFETY_CRITICAL
    if (perr == (INT8U*)0) { OS_SAFETY_CRITICAL_EXCEPTION(); return; }
#endif
#if OS_ARG_CHK_EN > 0u
    if (pevent == (OS_EVENT*)0) { *perr = OS_ERR_PEVENT_NULL; return; }
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_MUTEX) { *perr = OS_ERR_EVENT_TYPE; return; }
    if (OSIntNesting > 0u) { *perr = OS_ERR_PEND_ISR; return; }
    if (OSLockNesting > 0u) { *perr = OS_ERR_PEND_LOCKED; return; }

    OS_ENTER_CRITICAL();
    pcp = (INT8U)(pevent->OSEventCnt >> 8u);

    // =================================================================
    // [PART 1] 資源可用時：CPP 立即提升邏輯
    // =================================================================
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        pevent->OSEventCnt &= OS_MUTEX_KEEP_UPPER_8;

        // 【關鍵修正 1】先保存「原始」優先權，供 Post 還原用
        // 必須在修改 OSTCBCur->OSTCBPrio 之前執行這行！
        pevent->OSEventCnt |= OSTCBCur->OSTCBPrio;

        pevent->OSEventPtr = (void*)OSTCBCur;

        // 【關鍵修正 2】移除 "PCP Lower" 檢查
        // 因為在 CPP 巢狀鎖定中，允許高優先權任務(已提升過)去鎖低天花板資源

        // 【關鍵修正 3】執行立即提升 (Immediate Ceiling)
        if (pcp != OS_PRIO_MUTEX_CEIL_DIS) {
            if (OSTCBCur->OSTCBPrio > pcp) {
                // A. 從 Ready List 移除 (使用舊優先權)
                ...
                // B. 修改優先權為 Ceiling Priority
                OSTCBCur->OSTCBPrio = pcp;

                // C. 重新計算 TCB 的位置參數
#if OS_LOWEST_PRIO <= 63u
                OSTCBCur->OSTCBY = (INT8U)(OSTCBCur->OSTCBPrio >> 3u);
                OSTCBCur->OSTCBX = (INT8U)(OSTCBCur->OSTCBPrio & 0x07u);
#else
                OSTCBCur->OSTCBY = (INT8U)((INT8U)(OSTCBCur->OSTCBPrio >> 4u) & 0xFFu);
                OSTCBCur->OSTCBX = (INT8U)(OSTCBCur->OSTCBPrio & 0x0Fu);
#endif
                OSTCBCur->OSTCBBitY = (OS_PRIO)(1uL << OSTCBCur->OSTCBY);
                OSTCBCur->OSTCBBitX = (OS_PRIO)(1uL << OSTCBCur->OSTCBX);

                // D. 加回 Ready List (使用新優先權)
                OSRdyGrp |= OSTCBCur->OSTCBBitY;
                OSRdyTbl[OSTCBCur->OSTCBY] |= OSTCBCur->OSTCBBitX;

                OSTCBPrioTbl[pcp] = OSTCBCur;
            }
        }
    ...
    // =================================================================
    // [PART 2] 資源被佔用時：直接等待 (移除 PIP 提升邏輯)
    // =================================================================
    // 這裡移除了原本改變 mutex owner 優先權的程式碼，因為 CPP 不需要。
...
INT8U  OSMutexPost(OS_EVENT* pevent)
{
    INT8U      pcp;                                        /* Priority ceiling priority                      */
    INT8U      prio;
    BOOLEAN    prio_restored = OS_FALSE;                   /* [新增] 標記是否發生了優先權還原                 */
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register       */
    OS_CPU_SR  cpu_sr = 0u;
#endif
    ...
    // ------------------------------------------------------------------
    if (pcp != OS_PRIO_MUTEX_CEIL_DIS) {
        // 如果當前優先權等於 Ceiling，代表我們之前被提升過，現在要還原
        if (OSTCBCur->OSTCBPrio == pcp) {
            OS_TRACE_MUTEX_TASK_PRIO_DISINHERIT(OSTCBCur, prio);
            OSMutex_RdyAtPrio(OSTCBCur, prio);         /* Restore the task's original priority          */

            prio_restored = OS_TRUE;                   /* [關鍵] 標記：我們降低了自己的優先權            */
        }
        OSTCBPrioTbl[pcp] = OS_TCB_RESERVED;           /* Reserve table entry                           */
    }

    // ------------------------------------------------------------------
    // 檢查是否有其他任務在等待這個 Mutex
    // ------------------------------------------------------------------
    if (pevent->OSEventGrp != 0u) {
        ...
        // 這裡的 PCP Lower 檢查在標準 uC/OS-II 有，但在 CPP 中通常不會觸發，保留無妨
    ...
    // [關鍵新增] 如果我們剛剛降低了自己的優先權 (1 -> 6)，
    // 必須立刻呼叫排程器，因為可能有優先權 3 的任務 (Task 1) 正在等 CPU！
    if (prio_restored == OS_TRUE) {
        OS_Sched();
    }
