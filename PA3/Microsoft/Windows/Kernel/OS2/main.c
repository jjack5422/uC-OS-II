...
//                // 為了避免在同一個 tick 重複鎖定，通常需要一個 flag 或是確保只執行一次
//                // 這裡假設邏輯控制得當，只會在剛好等於該時間點觸發
//
...
//        //task_data->BlockTime = 0;
//        //task_data->PreemptiveTime = 0;
//    }
//    while (1) {}
//}
...
void task(void* p_arg) {
    ...
    INT32U current_time;
    INT32U executed_time;
    INT8U err; // [新增] 給 OSMutexPend/Post 使用的錯誤代碼變數
    ...
    while (stopAlltask == 0) {
        ...
        while (task_data->RemainTime > 0 && stopAlltask == 0) {
            ...
            if ((executed_time == task_data->R1Lock) && (task_data->R1UnLock != 0)) {
                ...
                OSSchedLock(); // 禁止搶佔
#elif PROTOCOL == CPP
                    // [計算 Current]: Lock R1 前，要檢查是否已持有 R2 (巢狀)
                ...
                // [計算 Target]: Lock R1 後，比較 current 與 R1 Ceiling
            ...
            if ((executed_time == task_data->R2Lock) && (task_data->R2UnLock != 0)) {
                ...
                OSSchedLock(); // 禁止搶佔 (巢狀鎖定，計數器+1)
#elif PROTOCOL == CPP
                // [計算 Current]: Lock R2 前，檢查是否已持有 R1
                // 你的案例 Task 1 在鎖 R2 時已經鎖了 R1，所以這裡會把 current 修正為 R1_Ceiling (2)
                current_prio_log = base_prio;
                ...
                // [計算 Target]: Lock R2 後，再跟 R2 Ceiling 比
            ...
            if ((executed_time == task_data->R1UnLock) && (task_data->R1UnLock != 0)) {
                ...
                OSSchedUnlock(); // 恢復排程 (計數器-1)
#elif PROTOCOL == CPP
                // [計算 Current]: Unlock 前，因為持有 R1，優先權必定包含 R1_Ceiling
                // 同時也要檢查是否持有 R2
                current_prio_log = base_prio;
                ...
                // [計算 Target]: Unlock 後，R1 移除。檢查剩下有沒有 R2
            ...
            if ((executed_time == task_data->R2UnLock) && (task_data->R2UnLock != 0)) {
                ...
                OSSchedUnlock(); // 恢復排程
#elif PROTOCOL == CPP
                // [計算 Current]: Unlock 前，因為持有 R2，優先權必定包含 R2_Ceiling
                // 同時檢查 R1
                current_prio_log = base_prio;
                ...
                // [計算 Target]: Unlock 後，R2 移除。檢查剩下有沒有 R1
                target_prio_log = base_prio;
                if (executed_time > task_data->R1Lock && executed_time < task_data->R1UnLock) {
...
int  main(void)
{
...
#elif PROTOCOL == CPP
    INT8U R1_Max_Prio = 255; // 數值越小優先權越高，所以初始設大
    INT8U R2_Max_Prio = 255;
    for (int i = 0; i < TASK_NUMBER; i++) {
        TaskParameter[i].TaskPriority = 3*(i+1);
    }

    for (int i = 0; i < TASK_NUMBER; i++) {
        // 檢查該 Task 是否使用 R1 (Lock 時間 > 0 即視為使用)
        if (TaskParameter[i].R1Lock < TaskParameter[i].R1UnLock) {
            if (TaskParameter[i].TaskPriority < R1_Max_Prio) {
                R1_Max_Prio = TaskParameter[i].TaskPriority;
            }
        }
        // 檢查該 Task 是否使用 R2
        if (TaskParameter[i].R2Lock < TaskParameter[i].R2UnLock) {
            if (TaskParameter[i].TaskPriority < R2_Max_Prio) {
                R2_Max_Prio = TaskParameter[i].TaskPriority;
            }
        }
    }

    // 根據 PDF [cite: 455, 456] 計算 Ceiling (Prio - Index)
    // 注意：uC/OS-II 的 PIP (Priority Inheritance Priority) 必須比任何持有該 Mutex 的 Task 優先權都高
    R1_Ceiling = R1_Max_Prio - 1;
    R2_Ceiling = R2_Max_Prio - 2;

    R1 = OSMutexCreate(R1_Ceiling, &err); // [cite: 510]
    R2 = OSMutexCreate(R2_Ceiling, &err); // [cite: 511]
    ...
    for (int n = 0; n < TASK_NUMBER; n++) {
        ...
        OSTaskCreateExt(task,
            &TaskParameter[n],
            &Task_STK[n][TASK_STACKSIZE - 1],
            TaskParameter[n].TaskPriority, // 設定好的 3, 6, 9...
            TaskParameter[n].TaskID,
            &Task_STK[n][0],
            TASK_STACKSIZE,
            &TaskParameter[n],
