                    ...
                    if (OSTCBCur->CompletedFlag == 1) {
                        ...
                        if (OSTCBHighRdy->OSTCBPrio != 63) {
                            LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID,
                                next->job_no,cur->ResponseTime-1, cur->ResponseTime - cur->TaskExecutionTime-1,cur->Delay);
                            cur->job_no++;
                        }
                        else {
                            LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t%d\t%d\t%d\n", OSTimeGet(), cur->TaskID, cur->job_no,
                                OSTCBHighRdy->OSTCBPrio,cur->ResponseTime-1, cur->ResponseTime - cur->TaskExecutionTime-1,cur->Delay);
                            cur->job_no++;
                        }
        ...
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSTCBCur->CompletedFlag == 1 && cur->Delay == 0) {
                if (OSTCBHighRdy == OSTCBCur) {
                    ...
                    //printf("%d\ttask1 arriveTime:%d\ttask1 Deadline:%d\n", OSTime, TaskParameter[0].TaskArriveTime, TaskParameter[0].deadline);
                    //printf("%d\ttask2 arriveTime:%d\ttask2 Deadline:%d\n", OSTime, TaskParameter[1].TaskArriveTime, TaskParameter[1].deadline);
                    OSTCBCur->CompletedFlag = 0;
                }
            }
            ...
            if (OSPrioHighRdy != OSPrioCur ) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if (OSTCBCur->CompletedFlag == 1) {
                    ...
                    if (OSTCBHighRdy->OSTCBPrio != 63) {
                        LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n", OSTimeGet(), cur->TaskID, cur->job_no, next->TaskID,
                            next->job_no,cur->ResponseTime, cur->ResponseTime - cur->TaskExecutionTime,cur->Delay);
                        cur->job_no++;
                    }
                    else {
                        LOG_print(3, "./Output.txt","%d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t%d\t%d\t%d\n", OSTimeGet(), cur->TaskID, cur->job_no, OSTCBHighRdy->OSTCBPrio,
                            cur->ResponseTime, cur->ResponseTime - cur->TaskExecutionTime,cur->Delay);
                        cur->job_no++;
                    }
                    OSTCBCur->CompletedFlag = 0;
...
static  void  OS_SchedNew(void)
{
    ...
    choosePrio = OS_TASK_IDLE_PRIO;
    earilestDeadline = 65535; // 最大值
    chosenTaskID = 63;
    ...
    while (ptcb != NULL && ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) 
    {
        ...
        task_para_set* task_params = (task_para_set*)(ptcb->OSTCBExtPtr);
        INT16U current_deadline = task_params->deadline;
        INT16U current_arrive_time = task_params->TaskArriveTime;
        INT16U current_task_id = task_params->TaskID;
        INT8U current_prio = ptcb->OSTCBPrio;
        
        // 1. 任務已到達 (currentTime >= current_arrive_time)
        if (currentTime >= current_arrive_time)
        {
            if (current_deadline < earilestDeadline)
            {
                // 發現更早截止的任務，直接替換
                earilestDeadline = current_deadline;
                chosenTaskID = current_task_id;
                choosePrio = current_prio;
            }
            // --- Secondary Condition: Same Deadline, Check TaskID ---
            else if (current_deadline == earilestDeadline)
            {
                // 截止時間相同，選 TaskID 較小的任務
                if (current_task_id < chosenTaskID)
                {
                    earilestDeadline = current_deadline; // 其實不必更新，但為了程式碼一致性可保留
                    chosenTaskID = current_task_id;
                    choosePrio = current_prio;
                }

                else if (current_task_id == chosenTaskID && ptcb == OSTCBCur)
                {
                    earilestDeadline = current_deadline;
                    chosenTaskID = current_task_id;
                    choosePrio = current_prio;
