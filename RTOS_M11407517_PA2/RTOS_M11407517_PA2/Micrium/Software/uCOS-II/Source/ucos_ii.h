...
#define MAX_PRINTF_BUFFER 16
    ...
    typedef struct task_para_set {
        ...
        INT16U TaskNumber;
        INT16U TaskPriority;
        INT16U job_no;                      //Job number
        INT16U RemainTime;                  //任務剩餘時間
        INT16U deadline;                    //任務deadline
        INT16U ResponseTime;                //紀錄response time
        INT16U Delay;                       //任務需要被OSTimeDly多久
    } task_para_set;

    typedef struct {
        INT16U time[MAX_PRINTF_BUFFER];      //紀錄時間
        INT8U  prev[MAX_PRINTF_BUFFER];      //context switch時紀錄前任務ID
        INT8U  cur[MAX_PRINTF_BUFFER];       //context switch時記錄目前任務ID
        INT16U rear;
        INT16U front;
    }BUFFER;
    ...
    typedef struct {
        ...
        INT32U AbsoluteDeadline; // Job deadline
        INT32U FinishedTime;     
        INT16U RemainTime;
        INT8U  IsArrived;       // 1: 時間到了，已抵達
        INT8U  IsFinished;      // 1: 做完了 (或被 Reject 了)
    } AperiodicJob;
    ...
    AperiodicJob AperTaskParameter[OS_MAX_TASKS];
