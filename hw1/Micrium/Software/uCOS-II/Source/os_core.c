        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    Buffer.prev[Buffer.front] = OSPrioCur;
                    if (Buffer.prev[Buffer.front] != 63)
                    {
                        Buffer.prev[Buffer.front] = ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskID;
                    }

                    Buffer.cur[Buffer.front] = OSPrioHighRdy;
                    if (Buffer.cur[Buffer.front] != 63)
                    {
                        Buffer.cur[Buffer.front] = ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskID;
                    }

                    Buffer.time[Buffer.front] = OSTimeGet();
                    Buffer.front = (Buffer.front + 1) % MAX_PRINTF_BUFFER;
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                //設定buffer存取Context Switch前的資料
                Buffer.prev[Buffer.front] = OSPrioCur;

                //如果Task不是Idle task(63) 存取OSTCBExtPtr TaskID的值
                if (Buffer.prev[Buffer.front] != 63) 
                {
                    Buffer.prev[Buffer.front] = ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskID;
                }

                //設定下個task，buffer存取Context Switch前的資料
                Buffer.cur[Buffer.front] = OSPrioHighRdy;
                if (Buffer.cur[Buffer.front] != 63)
                {
                    Buffer.cur[Buffer.front] = ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskID;
