...
void  App_TaskSwHook(void)
{
    ...
    OSProbe_TaskSwHook();
#endif
    //OS剛啟動還沒進行task 到TCB找HighRdy task
    if (Buffer.rear == Buffer.front) {
        printf("%d\t**********\ttask(%2d)(%2d)\t%d\t\n", OSTime, 
            ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskID, 
            ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->job_no, 
            count);
    }
    //代表進行context switch
    while (Buffer.rear != Buffer.front) {           
        int idx    = Buffer.rear;
        int time   = Buffer.time[idx];
        int prev   = Buffer.prev[idx];   //前一個任務taskID
        int cur    = Buffer.cur[idx];    //context switch完 準備執行的taskID


        if (prev != 63 && cur != 63) {
            printf("%d\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\n",
                time, prev, TaskParameter[prev - 1].job_no, cur, TaskParameter[cur - 1].job_no, count);
            TaskParameter[prev - 1].job_no++;
        }
        else if (prev == 63) {
            printf("%d\ttask(%2d)\ttask(%2d)(%2d)\t%d\n",
                time, prev, cur, TaskParameter[cur - 1].job_no, count);
        }
        else if (cur == 63) {
            printf("%d\ttask(%2d)(%2d)\ttask(%2d)\t%d\n",
                time, prev, TaskParameter[prev - 1].job_no, cur, count);
            TaskParameter[prev - 1].job_no++;
        }

        Buffer.rear = (Buffer.rear + 1) % MAX_PRINT_BUFFER;
        count++;
    }
}
